...
URL： 窗口内容页地址，或使用相对路径或绝对路径，注意如果使用http://www.5icool.org形式的绝对地址，则http://不能省略。
...
OnLoad：窗口内容载入完成后执行的程序，值为函数型。
  ...
  *
  **/
var IMAGESPATH = '/js/box/images/'; //图片路径配置
/*************************一些公用方法和属性****************************/
...
Dialog.prototype.autoClose = function () {
    ...
    this.AutoClose -= 1;
    topWin.$id("_Title_" + this.ID).innerHTML = this.AutoClose + " 秒";
    if (this.AutoClose <= 0) {
        this.close();
    } else {
        var self = this;
...
//弹出提示
    function box(type,msg){
        ...
        if(type=='ok'){
            diag.InnerHtml = "<p style='font-size:16px;'><img width='32px' height='32px' style='vertical-align:-8px;margin-right:4px;margin-top:10px;' src='"+IMAGESPATH+"ok.jpg'>"+"   "+msg+"<p>";
        }else if(type=='error'){          
            diag.InnerHtml = "<p style='font-size:16px;color:red'><img width='30px' height='30px' style='vertical-align:-8px;margin-right:20px;margin-top:10px;' src='"+IMAGESPATH+"error.jpg'>"+msg+"<p>";
        }
        diag.show();
    }
